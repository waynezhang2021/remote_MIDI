...
using namespace std;
double reference;
double hscale=4,vscale=4,offset=32;
int port=442;
SOCKET init()
{
	WSADATA wsaData;
...
COLORREF color(double a,double b)
{
	return RGB(sin(a)*64+127,sin(b)*64+127,sin(a+b)*64+127);
}
int main()
{
	int MSGSIZE=1048576;
	...
	while(1)
	{
		...
		while(ss>>eventbyte1>>eventbyte2>>eventbyte3)
		{
			if((hex_string_to_value(eventbyte1)&0xf0)==0x90)
				for(int i=hex_string_to_value(eventbyte2)*hscale; i<hex_string_to_value(eventbyte2)*hscale+hscale; i++)
					for(int j=(hex_string_to_value(eventbyte1)&0x0f)*vscale+offset; j<(hex_string_to_value(eventbyte1)&0x0f)*vscale+offset+vscale; j++)
						SetPixel(hdc,i,j,color(hex_string_to_value(eventbyte3)*3,hex_string_to_value(eventbyte1)*3));
			else if((hex_string_to_value(eventbyte1)&0xf0)==0x80)
				for(int i=hex_string_to_value(eventbyte2)*hscale; i<hex_string_to_value(eventbyte2)*hscale+hscale; i++)
					for(int j=(hex_string_to_value(eventbyte1)&0x0f)*vscale+offset; j<(hex_string_to_value(eventbyte1)&0x0f)*vscale+offset+vscale; j++)
